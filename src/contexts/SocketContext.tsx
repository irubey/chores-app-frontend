...
export const HouseholdProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    if (user && !hasFetchedHouseholds) {
      fetchHouseholds().then(() => {
        if (households.length === 0 && !['/', '/profile', '/household/create'].includes(pathname)) {
          router.push('/household/create');
        }
      });
    }
  }, [user, fetchHouseholds, hasFetchedHouseholds, households, router, pathname]);
        ...
        if (error.message === 'Not Found') {
          ...
          if (currentHousehold?.id === householdId) {
            ...
            if (remainingHouseholds.length > 0) {
              setCurrentHousehold(remainingHouseholds[0]);
            } else {
  ...
  const deleteHousehold = async (householdId: string) => {
    try {
      ...
      // Return the updated households length
  ...
  const value = useMemo(
    ...
    [households, currentHousehold, fetchHouseholds, createHousehold, inviteMember, isLoading, removeMember, joinHousehold, fetchHouseholdById, error, deleteHousehold, leaveHousehold]
  );
