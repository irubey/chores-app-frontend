...
import { Receipt } from '../../types/api'; // Ensure Receipt type is correctly imported
import { createSelector } from 'reselect';

// Define the FinancesState interface
...
export const fetchTransactions = createAsyncThunk<Transaction[], string, { rejectValue: string }>(
  ...
  async (householdId, thunkAPI) => {
    try {
      const response = await apiClient.finances.getTransactions(householdId);
      return response.data;
    } catch (error: any) {
      const message =
        error.response?.data?.message ||
...
export const addTransaction = createAsyncThunk<Transaction, { householdId: string; transactionData: CreateTransactionDTO }, { rejectValue: string }>(
  'finances/addTransaction',
  async ({ householdId, transactionData }, thunkAPI) => {
    try {
      const response = await apiClient.finances.createTransaction(householdId, transactionData);
      return response.data;
    } catch (error: any) {
      const message =
        error.response?.data?.message ||
...
export const updateTransaction = createAsyncThunk<Transaction, { householdId: string; transactionId: string; transactionData: UpdateTransactionDTO }, { rejectValue: string }>(
  'finances/updateTransaction',
  async ({ householdId, transactionId, transactionData }, thunkAPI) => {
    try {
      const response = await apiClient.finances.updateTransaction(householdId, transactionId, transactionData);
      return response.data;
    } catch (error: any) {
      const message =
        error.response?.data?.message ||
...
export const deleteTransaction = createAsyncThunk<string, { householdId: string; transactionId: string }, { rejectValue: string }>(
...
export const addReceipt = createAsyncThunk<
  { expenseId: string; receipt: Receipt },
  { householdId: string; expenseId: string; receiptFile: File },
  { rejectValue: string }
>(
  'finances/addReceipt',
  async ({ householdId, expenseId, receiptFile }, thunkAPI) => {
    try {
      const response = await apiClient.finances.uploadReceipt(householdId, expenseId, receiptFile);
      return { expenseId, receipt: response.data };
    } catch (error: any) {
      const message =
        error.response?.data?.message ||
        error.message ||
...
export const selectTotalDebts = createSelector(
  [selectExpenses],
  (expenses) => expenses.reduce((total, expense) => 
    total + expense.splits.reduce((splitTotal, split) => splitTotal + split.amount, 0), 0)
);
  ...
  (transactions) => {
    ...
    transactions.forEach(transaction => {
      if (!transactionSummaries[transaction.fromUserId]) {
        transactionSummaries[transaction.fromUserId] = { totalSent: 0, totalReceived: 0, netBalance: 0 };
      }
      if (!transactionSummaries[transaction.toUserId]) {
        transactionSummaries[transaction.toUserId] = { totalSent: 0, totalReceived: 0, netBalance: 0 };
      }

      if (transaction.status === TransactionStatus.COMPLETED) {
        transactionSummaries[transaction.fromUserId].totalSent += transaction.amount;
        transactionSummaries[transaction.toUserId].totalReceived += transaction.amount;
      }
    });

    Object.keys(transactionSummaries).forEach(userId => {
      transactionSummaries[userId].netBalance = 
        transactionSummaries[userId].totalReceived - transactionSummaries[userId].totalSent;
    });
...
export const selectFinances = createSelector(
  ...
  (finances, totalExpenses, totalDebts, userSummaries, transactionSummaries) => ({
    ...finances,
    totalExpenses,
    totalDebts,
    userSummaries,
...
const financesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(addReceipt.fulfilled, (state, action: PayloadAction<{ expenseId: string; receipt: Receipt }>) => {
        state.isUploadingReceipt = false;
        const { expenseId, receipt } = action.payload;
        if (!state.receipts[expenseId]) {
          state.receipts[expenseId] = [];
        }
        state.receipts[expenseId].push(receipt);
      })
...
// Add new selectors
export const selectReceiptsByExpenseId = (state: RootState, expenseId: string) => state.finances.receipts[expenseId] || [];
export const selectIsUploadingReceipt = (state: RootState) => state.finances.isUploadingReceipt;
export const selectUploadReceiptError = (state: RootState) => state.finances.uploadReceiptError;
